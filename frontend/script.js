...
function handleFileSelect(file) {
  ...
  reader.onload = (e) => {
    ...
    uploadSection.querySelectorAll('.upload-subtext').forEach(el => el.style.display = 'none');
  };
...
async function processImage() {
  ...
  imageResult.innerHTML = '<div class="image-placeholder">Processing...</div>';
...
function displayJsonResult(data) {
  ...
  // Show detection list
  let html = `<div class="detections-summary">Detected ${count} object${count !== 1 ? 's' : ''}</div>`;
      ...
      html += `
        <div class="detection-item">
          <div class="detection-header">
            <span class="detection-class">#${detection.index} ${detection.class}</span>
            <span class="detection-confidence">${confidence}%</span>
          </div>
          <div class="detection-bbox">
            Box: [${Math.round(bbox.x1)}, ${Math.round(bbox.y1)}, ${Math.round(bbox.x2)}, ${Math.round(bbox.y2)}]
          </div>
        </div>
      `;
    });
  ...
  html += '<div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #3a4055;">';
  html += '<div style="color: #b0b3b8; font-size: 0.85rem; margin-bottom: 10px; font-family: system-ui;">Raw JSON:</div>';
