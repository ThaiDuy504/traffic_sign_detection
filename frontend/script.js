...
function displayJsonResult(data) {
  els.jsonResult.innerHTML = `<pre>${syntaxHighlight(JSON.stringify(data, null, 2))}</pre>`;
}
...
async function processImage() {
  ...
  try {
    ...
    if (!detectRes.ok) {
      throw new Error((await detectRes.json()).detail || "Detection failed");
    }
    if (!imgRes.ok) {
  ...
  try {
    ...
    state.videoSocket.onmessage = (event) => {
      ...
      if (typeof event.data === "string") {
        ...
        const progress =
          total_frames > 0 ? Math.round((frameCount / total_frames) * 100) : 0;
        setLoading(
          true,
...
async function startCamera() {
  try {
    ...
    function processFrame() {
      ...
      if (state.cameraVideo.readyState === state.cameraVideo.HAVE_ENOUGH_DATA) {
        const scaleFactor = 0.5;
        canvas.width = state.cameraVideo.videoWidth * scaleFactor;
        canvas.height = state.cameraVideo.videoHeight * scaleFactor;
        ...
        canvas.toBlob(
          (blob) => {
            if (blob && state.cameraSocket.readyState === WebSocket.OPEN) {
